  ...
  if(all == 1 || (!strcmp(c,"-expr")))
    {
      ...
      printtostring(&s,"Evaluate an analytic expression. The argument to this command is an equality, with the left-hand-side being the name of a variable to update, and the right-hand-side being an analytic expression. See \"vartools -functionlist\" for the allowed syntax. If the variable on the left-hand-side has not previously been defined, it will be created. Note that variables which appear on the right-hand-side can be the name of a vector which is read-in from the light curve and set using the -inputlcformat option, a scalar of vector created by another command (e.g. the fitting parameters, or the output model vector, created by the -linfit command), or any output parameter from a previously executed command (these latter variables have the names of the output columns which can be found by running vartools with the -headeronly command. Note that any leading numbers or '_' characters are removed). The left-hand-side cannot be a variable associated with an output column.\n\nUse square brackets '[]' after the variable name on the left-hand-side to update only a subset of the components of the vector. Within the square brackets you can either specify a single index value (indexed starting from 0), an expression that evaluates to a single index value, a range of indices (using a ':' to distinguish between the first index and the last index to operate on) or an expression that evaluates to a vector, only components where the resulting vector has a value > 0 will be updated. Examples of each of these are is follows: '-expr mag[0]=5.0' would set the magnitude value for the first observation in the light curve to 5.0, and would not affect any of the other components in the light curve. '-expr mag[i+2]=5.0' would set the magnitude value for the observation indexed by 'i+2' (assuming 'i' is not a light curve vector itself) to 5.0. '-expr mag[len(mag)-2]=5.0' would set the magnitude value for the second to last observation to 5.0. Note that len(mag) returns the length of the magnitude vector, and due to indexing from zero, len(mag)-1 is the index of the last point in the vector. '-expr mag[0:2]=5.0' would set the magnitude values for the first and second observations in the light curve to 5 (note that here we follow the python convention where mag[0:2] corresponds to mag[0] and mag[1]). '-expr mag[:5]=5.0' would set the magnitude values for the first five observations in the light curve to 5. '-expr mag[5:]=5.0' would set the magnitude values for all but the first five observations to 5.0. '-expr mag[(t>25.0)&&(t<30.0)]=5.0' would set the magnitude values for any points with 25 < t < 30.0 to 5.0. '-expr mag[t-25.0]=5.0' would set the magnitude values for any points with t > 25 to 5.0.\n\n");
      commandfound = 1;
    }
