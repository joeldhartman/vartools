  ...
  if(!strncmp(c,"-BLS",4) && strlen(c) == 4)
    {
      ...
      printtostring(&s,
		    "\nvartools -i EXAMPLES/3.transit -oneline \\\n");
      printtostring(&s,
		    "\t-BLS q 0.01 0.1 0.5 5.0 nf 20000 200 0 3 \\\n");
      printtostring(&s,
		    "\t\t1 EXAMPLES/OUTDIR1/ 0 0 medsn medwindow 0.5 innerN 5 outerN 100\n\n");
      printtostring(&s,
		    "This example illustrates the optional \"medsn\" keyword, which replaces the BLS_SN statistic with a robust median-filter-based signal-to-noise. The SR periodogram is first detrended by subtracting a moving median computed over a 0.5 cycles/day window. For each of the 3 reported peaks BLS_SN is then reported as (peak - local_mean)/(1.4826*MAD), where local_mean is the mean of the detrended spectrum over the side-bands from 5/T to 100/T on either side of the peak (T is the light-curve time baseline), and MAD is the median absolute deviation of the whole detrended spectrum. BLS_SR still reports the raw signal residue and the peak selection is unchanged; the diagnostic columns BLS_MedFiltPeakHeight, BLS_MedFiltLocalMean and BLS_MedFiltNoise give the components of the S/N. The S/N column of the output periodogram EXAMPLES/OUTDIR1/3.transit.bls is also the median-filter S/N. Add the \"useforpeaks\" keyword to select and order the peaks by this S/N rather than by the default BLS statistic.\n");
      commandfound=1;
    }
  ...
  if(!strcmp(c,"-BLSFixDurTc"))
    {
      ...
      printtostring(&s,
		    "\t-rms\n\n");
      printtostring(&s,
		    "Run a BLS search on the light curve EXAMPLES/3.transit fixing the duration of the transit to 0.076996297 and the transit epoch to 53727.29676321477. Allow for periods between 0.1 and 20. days, using 100000 frequency steps. Set the time-zone to 0 (this only affects the BLSFixPer_fraconenight statistic. Do not output the BLS periodogram or the model, but subtract the best-fit model from the light curve before passing it on to the next command (in this example, that is the final -rms command). The fittrap option causes the procedure to fit a trapezoid-shaped transit. The calls to -rms before and after -BLSFixDurTc show how subtracting the BLS model reduces the scatter of the light curve. Like -BLS, -BLSFixDurTc also accepts the optional \"medsn\" keyword to report BLSFixDurTc_SN as a robust median-filter-based signal-to-noise (see the -BLS example).\n");
      commandfound=1;
    }
