...
/* Output the help information for this command */
{
  ...
  VARTOOLS_printtostring(&s,"mask_variable_list - A comma-separated list of variables to use for masking points from the fitting procedure. This list must have the same number of elements as the stitch_variable_list. Points with a mask value greater than 0 will be included in the fit, while points with a mask value of 0 (or negative) will be excluded from the fit. By default an excluded point still has the fitted per-segment shift applied to it (see the \"noshiftmasked\" keyword to change this).\n\n");
